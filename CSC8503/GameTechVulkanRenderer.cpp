...
GameTechVulkanRenderer::GameTechVulkanRenderer(GameWorld& world) :/* VulkanRenderer(*Window::GetWindow()),*/ gameWorld(world) {
	...
	*/
	...
	renderer = new VulkanRenderer(*Window::GetWindow(), vkInit);
...
void	GameTechVulkanRenderer::InitStructures() {
	...
	globalDataLayout = DescriptorSetLayoutBuilder(renderer->GetDevice())
		.WithUniformBuffers(1, 1, vk::ShaderStageFlagBits::eAll)	//0: The global data uniform buffer
		.WithStorageBuffers(1, 1, vk::ShaderStageFlagBits::eAll)	//1: The list of object states
		.WithSamplers(1, 1, vk::ShaderStageFlagBits::eAll)			//2: The scene shadow map
		.WithSamplers(1, 1, vk::ShaderStageFlagBits::eAll)			//3: The scene cubemap

		.Build("Global Data Layout");

	objectTextxureLayout = DescriptorSetLayoutBuilder(renderer->GetDevice())
		.WithSamplers(0, _TEXCOUNT)		//0: Object textures live here
		.Build("Object Texture Layout");

	objectTextxureDescriptor = BuildUniqueDescriptorSet(*objectTextxureLayout);

	defaultTexture = TextureBuilder(renderer->GetDevice(), renderer->GetMemoryAllocator())
		.WithPool(renderer->GetCommandPool(CommandType::Graphics))
		.WithQueue(renderer->GetQueue(CommandType::Graphics))
		.BuildFromFile("Default.png");

	loadedTextures.push_back(&(*defaultTexture)); //Default will always be index 0!

	defaultSampler = renderer->GetDevice().createSamplerUnique(
	...
	textSampler = renderer->GetDevice().createSamplerUnique(
		vk::SamplerCreateInfo()
		.setAnisotropyEnable(false)
		.setMaxAnisotropy(1)
	...
	{//Setting up the data for the skybox
		...
		cubeTex = TextureBuilder(renderer->GetDevice(), renderer->GetMemoryAllocator())
			.WithPool(renderer->GetCommandPool(CommandType::Graphics))
			.WithQueue(renderer->GetQueue(CommandType::Graphics))
		...
		skyboxPipeline = PipelineBuilder(renderer->GetDevice())
	...
	{//Setting up all the data we need for shadow maps!
		shadowShader = ShaderBuilder(renderer->GetDevice())
			.WithVertexBinary("Shadow.vert.spv")
			.WithFragmentBinary("Shadow.frag.spv")
			.Build("Shadow Fill Shader");

		shadowMap = TextureBuilder(renderer->GetDevice(), renderer->GetMemoryAllocator())
			.WithPool(renderer->GetCommandPool(CommandType::Graphics))
			.WithQueue(renderer->GetQueue(CommandType::Graphics))
	...
	{//
		sceneShader = ShaderBuilder(renderer->GetDevice())
	...
	for (int i = 0; i < _FRAMECOUNT; ++i) {
		allFrames[i].fence = renderer->GetDevice().createFenceUnique({});

		{//We store scene object matrices in a big UBO
			allFrames[i].debugVertSize = 10000;
			allFrames[i].objectCount = 1024;

			allFrames[i].dataBuffer = BufferBuilder(renderer->GetDevice(), renderer->GetMemoryAllocator())
...
void GameTechVulkanRenderer::BuildScenePipelines(VulkanMesh* m) {
	...
	scenePipeline = PipelineBuilder(renderer->GetDevice())
		...
		.WithColourAttachment(frameState.colourFormat, vk::BlendFactor::eSrcAlpha, vk::BlendFactor::eOneMinusSrcAlpha)
...
void GameTechVulkanRenderer::BuildDebugPipelines() {
	...
	debugLineShader = ShaderBuilder(renderer->GetDevice())
		.WithVertexBinary("Debug.vert.spv")
		.WithFragmentBinary("Debug.frag.spv")
		.Build("Debug Line Shader");

	debugTextShader = ShaderBuilder(renderer->GetDevice())
	...
	debugLinePipeline = PipelineBuilder(renderer->GetDevice())
	...
	debugTextPipeline = PipelineBuilder(renderer->GetDevice())
...
void GameTechVulkanRenderer::RenderFrame() {
	...
	TransitionUndefinedToColour(frameState.cmdBuffer, frameState.colourImage);
	...
	frameData.shadowMatrix  =	Matrix::Perspective(50.0f, 500.0f, 1, 45.0f) * 
	...
	{//Render the shadow map for the frame	
		DynamicRenderBuilder()
			...
			.BeginRendering(frameState.cmdBuffer);

		frameState.cmdBuffer.setViewport(0, 1, &shadowViewport);
		frameState.cmdBuffer.setScissor(0, 1, &shadowScissor);

		RenderSceneObjects(shadowPipeline, frameState.cmdBuffer); //Draw the shadow map

		frameState.cmdBuffer.endRendering();
	}
	
	//Now render the main scene view
	TransitionDepthToSampler(frameState.cmdBuffer, *shadowMap);

	frameState.cmdBuffer.setViewport(0, 1, &frameState.defaultViewport);
	frameState.cmdBuffer.setScissor(0, 1, &frameState.defaultScissor);

	DynamicRenderBuilder()
	.WithColourAttachment(frameState.colourView)
	.WithDepthAttachment(frameState.depthView)
	.WithRenderArea(frameState.defaultScreenRect)
	.BeginRendering(frameState.cmdBuffer);

	RenderSkybox(frameState.cmdBuffer);
	RenderSceneObjects(scenePipeline, frameState.cmdBuffer); //Draw the main scene

	UpdateDebugData();
	RenderDebugLines(frameState.cmdBuffer);
	RenderDebugText(frameState.cmdBuffer);

	frameState.cmdBuffer.endRendering();

	TransitionSamplerToDepth(frameState.cmdBuffer, *shadowMap);
...
void GameTechVulkanRenderer::RenderSceneObjects(VulkanPipeline& pipe, vk::CommandBuffer cmds) {
	...
	for (int i = 1; i < activeObjects.size(); ++i) {
		...
		if (activeMesh != objectMesh) {
			...
			renderer->DrawMesh(cmds, *activeMesh, count);
		...
		if (i == activeObjects.size()-1) {
			cmds.pushConstants(*pipe.layout, vk::ShaderStageFlagBits::eVertex, 0, sizeof(uint32_t), (void*)&startingIndex);
			renderer->DrawMesh(cmds, *objectMesh, count);
		}
		else {
			count += increment;
		}
...
void GameTechVulkanRenderer::RenderSkybox(vk::CommandBuffer cmds) {
	...
	renderer->DrawMesh(cmds, *quadMesh);
}
...
Texture* GameTechVulkanRenderer::LoadTexture(const string& name) {
	VulkanTexture* t = TextureBuilder(renderer->GetDevice(), renderer->GetMemoryAllocator())
		.WithPool(renderer->GetCommandPool(CommandType::Graphics))
		.WithQueue(renderer->GetQueue(CommandType::Graphics))
