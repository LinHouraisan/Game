...
namespace NCL::Maths {
	...
	public:
		...
		static Quaternion AxisAngleToQuaternion(const Vector3& axis, float degrees) {
			float halfAngleRad = Maths::DegreesToRadians(degrees) * 0.5f;
			float sinHalfAngle = sin(halfAngleRad);
			Vector3 normalizedAxis = Vector::Normalise(axis);
			Quaternion resulet = Quaternion(
				normalizedAxis.x * sinHalfAngle,
				normalizedAxis.y * sinHalfAngle,
				normalizedAxis.z * sinHalfAngle,
				cos(halfAngleRad));
			return resulet.Normalised();
		}
